...
namespace WarmPenguin.StorageBroker
{
    ...
    public class DealData
    {
        ...
        private static async Task<Deal[]> PopulateDeals(IEnumerable<XElement> properties)
        {
            List<Deal> deals = new List<Deal>();

            foreach (var item in properties)
            {
                deals.Add(await PopulateDeal(item));
            }

            return deals.ToArray();
        }

        private static async Task<Deal> PopulateDeal(XElement properties)
        { 
            Deal deal = new Deal();

            deal.DealId = (string)properties.Element(DataFeed.xd + "RowKey");
            deal.Title = (string)properties.Element(DataFeed.xd + "Title");
            deal.Description = (string)properties.Element(DataFeed.xd + "Description");
            deal.BeginDeal = (DateTime)properties.Element(DataFeed.xd + "BeginDeal");
            deal.EndDeal = (DateTime)properties.Element(DataFeed.xd + "EndDeal");
            deal.SavingsPercentage = (int)properties.Element(DataFeed.xd + "SavingsPercentage");
            deal.StockAmount = (int)properties.Element(DataFeed.xd + "StockAmount");
            deal.Price = (double)properties.Element(DataFeed.xd + "Price");
            deal.CountryRowKey = (string)properties.Element(DataFeed.xd + "CountryRowKey");
            deal.Redwine = (bool)properties.Element(DataFeed.xd + "Redwine");
            deal.White = (bool)properties.Element(DataFeed.xd + "WhiteWine");
            deal.CategoryRowKey = (string)properties.Element(DataFeed.xd + "CategoryRowKey");
			deal.ImageUrl = (string)properties.Element(DataFeed.xd + "ImageUrl");
        ...
        {
            var data = await DataFeed.GetStringAsync("Deals?$filter=(CountryRowKey eq '" + countryId + "') and (CategoryRowKey eq '"+ categoryId +"')");
    ...
    public class Category : INotifyPropertyChanged
    {
        ...
        public int DealCount
        {
            ...
            set
            {
                NotifyPropertyChanged("DealCount");
